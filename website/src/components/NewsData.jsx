            ...
            <div className="flex flex-wrap gap-4 w-full h-full justify-between">
                { loading ? "Loading..." : 
                    arr.map(num => {
                        ...
                        return <NewsArticle date = {date_format(article.publishedAt)} url = {article.url} urlToImage={article.urlToImage} title={article.source.name + ": " + title_format(article.title)}/>
                    })
                }
            </div>
    )
